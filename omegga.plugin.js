module.exports = class Eval {
  ...
  async loadModule(name) {
    return this.modules[name] = await require('./modules/' + name + '.js')(this, global.OMEGGA_UTIL);
  }

  // add a module to the ones that will be loaded
  async addModule(name, load=false) {
    await this.store.set('modules', (await this.store.get('modules') || []).concat([name]));
    if (load)
      return this.modules[name] = await this.loadModule(name);
    return 'ok';
  }

  // add a module to the ones that will be loaded
  async removeModule(name) {
    await this.store.set('modules', (await this.store.get('modules') || []).filter(m => m.name !== name));
    return 'ok';
  }
  ...
  async init() {
    ...
    if (loaded > 0)
      console.info('Loaded', loaded, 'modules');
